...
static bool WriteLines(ofstream& ofs, LEN start, LEN end, string ifile)
{
    int h = 0;
    DOUBLE value;
    ...
    // ifs.seekg(sizeof(int)*start+sizeof(DOUBLE)*(start)*h, std::ios_base::beg);
    for (LEN i = start; i < end; i++) {
        ifs.read((char*)&h, sizeof(int));
        if (CheckHeader(ifs, h, ifile)) return true;
        vector<DOUBLE> vec;
        for (int j = 0; j < h; j++) {
            ifs.read((char*)&(value), sizeof(DOUBLE));
            vec.push_back(value);
...
static bool ConcatBinReverse(string filename)
{
    ...
    for (LEN i = all; i >= 0; i -= step) {
        if (WriteLines(ofs, max((LEN)0, i-step), i, filename)) return true;
    }
    ifs.close();
    ofs.close();
    MoveFile(filename, tfile);
